...
needs an `apps/api/.env.local` (generated by Neon, with two keys appended by hand) plus
two small per-app `.env` files (hand-written once, gitignored):
...
2. Append the two app-local keys to that same `apps/api/.env.local` (`neon env pull`
   only rewrites Neon-managed keys, so they survive later pulls):
...
Both `.env` files and `apps/api/.env.local` are covered by `.gitignore`'s `.env` and
`*.local` rules — never commit any of them. Each app also ships an `.env.example` with
the same keys, in the same order, for reference.
...
- Auth is required in every environment: when neither `PUBLISH_TOKEN` nor
  ...
  dev-mode bypass, so a local `apps/api/.env.local` needs at least `NEON_AUTH_ORIGIN`
  set and the repo linked to Neon (see **Local dev setup**) before the CMS can do
  anything.
