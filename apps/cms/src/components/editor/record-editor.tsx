      ...
      <ScrollArea className="min-h-0 flex-1">
        <EditorSection title="Basic info">
          {collection.fields.slice(0, 3).map((field) => (
            <FieldControl field={field} key={field.key} onAssetUpload={onAssetUpload} onUpdateValue={onUpdateValue} record={draftRecord} uploadingField={uploadingField} />
          ))}
        </EditorSection>

        <EditorSection title="Custom fields">
        ...
        <EditorSection title="Activity">
          <div className="grid gap-3 text-[11px] text-cms-muted">
            <p className="m-0">
              <span className="mr-2 inline-block size-[18px] rounded-full border-2 border-cms-accent bg-cms-raised align-middle" /> Item created{" "}
              {formatDateTime(draftRecord.createdAt)} by Local Editor
            </p>
            <p className="m-0">
              <span className="mr-2 inline-block size-[18px] rounded-full border-2 border-cms-accent bg-cms-raised align-middle" /> Last edited{" "}
              {formatDateTime(draftRecord.modifiedAt)} by Local Editor
            </p>
          </div>
        </EditorSection>
      </ScrollArea>
